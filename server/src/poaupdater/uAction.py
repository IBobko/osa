...
def ntimes_retried(fun, n=3, timeout=30):
    def fn(*args, **kwargs):
        for i in range(1, n + 1):
            uLogging.info('N-times retried action "{action_name}", try {i} of {n}...'.format(
                action_name=fun.__name__, i=i, n=n))
            try:
                return fun(*args, **kwargs)
            except Exception, e:

                if i >= n:
                    uLogging.err('Failed n-times retried action "{action_name}"')
                    raise e

                uLogging.debug('Got exception in action "{action_name}", retrying after timeout {timeout} sec. '
                               'Exception message: \n{e}'.format(action_name=fun.__name__, timeout=timeout, e=e))
                time.sleep(timeout)
    return fn
    ...
    def fn(*args, **kwds):
        ...
        while True:
            try:
                ...
                if progress.what():
                    progress_definition = 'in "{progress_name}"'.format(progress_name=progress.what())

                uLogging.debug("\n\n{exc_name} occurred {progress_definition}: {err_msg}".format(
                    exc_name = e.__class__.__name__, progress_definition=progress_definition, err_msg=err_msg))
                answer = None
                ...
                while not answer:
                    ...
                    if abort_pattern.match(answer):
                        uLogging.warn("Action {progress_definition} has been ABORTED".format(
                            progress_definition=progress_definition))
                        raise
                    elif ignore_pattern.match(answer):
                        uLogging.warn("Action {progress_definition} has been IGNORED".format(
                            progress_definition=progress_definition))
                        if raise_on_ignore:
                            raise ActionIgnored()
                        else:
                        ...
                        uLogging.warn("Retrying action {progress_definition} ...".format(
                            progress_definition=progress_definition))
                        pass
