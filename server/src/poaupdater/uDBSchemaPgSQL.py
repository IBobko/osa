...
def _getTable(name, con):
    ...
    cur.execute("""SELECT trs.tgname AS \"TriggerName\"
                             FROM pg_trigger AS trs
                             INNER JOIN pg_class AS cls ON trs.tgrelid=cls.oid WHERE NOT trs.tgisinternal AND trs.tgenabled <> 'D' AND cls.relname = %s""", (name,))

    triggers = []
    for row in cur.fetchall():
        triggers.append(row['TriggerName'])

    cur.close()
    return Table(name, columns, constraints, indexes, referenced_fk, triggers)
...
def dropOwnedBy(con, db_user):
    cur = con.cursor()
    cur.execute("DROP OWNED BY %s CASCADE" % db_user)
