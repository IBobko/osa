...
corerpms_dir_pattern = re.compile(r'^.*(RHEL)([\d\.]+)')
...
def add_files(ctx, dirname, files):
    ...
    build = ctx[0]
    files_filter = ctx[1]
    os_dir_pattern = ctx[2]
    pmatch = os_dir_pattern.match(dirname)
    if pmatch:
...
def read_u_folder(build):
    ...
    if os.path.exists('u/pau/core-ear-DB-MIGRATION.zip'):
        build.add_pau_db_migration(os.path.abspath('u/pau'))
...
def getBuildFromFS(path, buildname, conf=None, fail_on_duplicates=False, calculateChecksums=True, filesFilter=None, os_dir_pattern=platform_dir_pattern):
    """
    ...
    try:
        os.chdir(path)
        os.path.walk(".", add_files, (rv, filesFilter, os_dir_pattern))
        read_u_folder(rv)
    finally:
        os.chdir(curdir)
...
def _getCumulativeAgents(agents_dir):
    ...
    if os.path.exists(agents_path):
        ...
        return getBuildFromFS(agents_path, os.path.basename(os.path.realpath(agents_path)), None, False, False, None, corerpms_dir_pattern)
    else:
