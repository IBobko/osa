...
class Script(Action):
    ...
    def __init__(self, id, owner, script, validate=True):
        Action.__init__(self, id)
        self.owner = owner
        ...
        self.script = script
        if not self.script or (validate and not os.path.exists(self.script)):
...
class SQLScript(Script):
    ...
    def __init__(self, id, owner, script, validate=True):
        Script.__init__(self, id, owner, script, validate)
...
class UpdBase:
    ...
    def add_script_from_node(self, root_dir, node):
        ...
        kind = node.tagName
        if "EXEC" == kind:
            self.actions.append(ExecScript(action_id, action_owner, script, self.validate))
        elif "SQL" == kind:
            self.actions.append(SQLScript(action_id, action_owner, script, self.validate))
        elif "INSTRUCTION" == kind:
            self.actions.append(Instruction(action_id, action_owner, script, self.validate))
        else:
...
class Update(UpdBase):
    ...
    def __init__(self, validate=True):
        UpdBase.__init__(self, validate)

        self.name = ''
        self.version = ''
        self.preparation = UpdBase(validate)
        self.pre = UpdBase(validate)
        self.prepkg = UpdBase(validate)
        self.post = UpdBase(validate)
        self.patches = {}
        self.aps = []
        self.current_update = None

        self.update_windows = False
        self.cleanup = UpdBase(validate)
    ...
    def add_helm_package(self, pkg):
        uLogging.debug('Distributive has Helm package: %s' % pkg)
        self.helm_packages.append(pkg)

    def add_node_package(self, root_dir, node):
        if node.getAttribute('ctype') == 'helm':
            self.add_helm_package(parseHelmPackageNode(root_dir, node))
        elif node.getAttribute('host') == 'no_auto':
    ...
    def save_helm_packages(self, doc, node):
        for pkg in self.helm_packages:
            elem = doc.createElement("PACKAGE")
            elem.setAttribute("ctype", "helm")
            elem.setAttribute("host", pkg.host)
            elem.setAttribute("name", pkg.name)
            elem.setAttribute("chart", pkg.chart)
            if pkg.version:
                elem.setAttribute("version", pkg.version)
            elem.setAttribute("group", pkg.group)
            if pkg.script:
                elem.setAttribute("script", os.path.basename(pkg.script))
    ...
    def save(self, to, prechecks_only=False):
        ...
        self.save_helm_packages(doc, root)
...
class BuildInfo:
    ...
    def __init__(self, builds):
        ...
        self.helms_to_update = {}           # merged Helm packages to update: {'foundation': [...], 'core': [...], 'optional': [...]}
...
def getOrderedUdlFiles(udl_files, present_updates, required_updates):
    ...
    return [x[1] for x in name_file]


def _getMaxHelmPackage(helm_packages):
    return sorted(helm_packages, cmp=lambda x,y: uBuild.compare_versions(x.version, y.version), reverse=True)[:1][0]


def _getCumulativeHelmPackages(helm_packages):
    getKey = lambda pkg: (pkg.chart, pkg.name)
    uniq_packages = sorted(helm_packages, key=getKey)
    uniq_packages = [_getMaxHelmPackage(list(pkgs)) for _, pkgs in itertools.groupby(uniq_packages, getKey)]
    return list(filter(lambda pkg: (pkg in uniq_packages), helm_packages)) # keep order


def _getCumulativeHelmPackagesGroups(helm_packages):
    getKey = lambda pkg: pkg.group
    helm_packages = sorted(helm_packages, key=getKey)
    return dict((group, _getCumulativeHelmPackages(list(pkgs))) for group, pkgs in itertools.groupby(helm_packages, getKey))


def _getCumulativeUpgradeInstructions(ordered_udlfiles, validate):
    update_builder = UpdateBuilder(validate)
    version_list = [update_builder.read_and_validate(doc) for doc in ordered_udlfiles]
    upgrade_instructions = update_builder.product()
    return version_list, upgrade_instructions


def getCumulativeUpgradeInstructions(udl_files):
    ordered_udl_files = getOrderedUdlFiles(udl_files, set(), set())
    _, upgrade_instructions = _getCumulativeUpgradeInstructions(ordered_udl_files, validate=False)
    return upgrade_instructions


def getCumulativeUpdateInstructions(builds):
    filelist = dict([(build.udl2_file, build) for build in builds])
    present_updates = set()
    required_updates = set()
    ordered_udlfiles = getOrderedUdlFiles(filelist.keys(), present_updates, required_updates)
    ordered_builds = []
    ...
    for x in ordered_udlfiles:
        ...
        if build.jboss.pau_db_migration:
            jboss_components.pau_db_migration = build.jboss.pau_db_migration
        if build.jboss.pui:
    ...
    version_list, upgrade_instructions = _getCumulativeUpgradeInstructions(ordered_udlfiles, validate=True)
    binfo.version_list.extend(version_list)
    binfo.upgrade_instructions = upgrade_instructions

    binfo.helms_to_update = _getCumulativeHelmPackagesGroups(upgrade_instructions.helm_packages)
    for _, helm_packages in binfo.helms_to_update.iteritems():
        for helm_package in helm_packages:
            uLogging.debug('Selected Helm package to update: %s' % helm_package)
    return binfo
