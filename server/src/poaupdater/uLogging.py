...
initial_thread_id = thread.get_ident()
...
def print_log(stream, prio, message, *msg_args):
    ...
    printing_thread_id = thread.get_ident()

    if printing_thread_id != initial_thread_id:
        msg = "[{printing_thread_id}] {msg}".format(printing_thread_id=printing_thread_id, msg=msg)
...
def save_traceback():
    import traceback
    debug("Traceback saved: \n{exc_info} \n{format_exc}".format(
        exc_info=sys.exc_info(), format_exc=traceback.format_exc()))
