...
def setup_connection():
    ...
    if _config.database_type == 'MSSQL':
        ...
        def get_connection(_config, uname):
            import psycopg2 as _dbmodule

            return PyFormatConnection(
                    _dbmodule.connect(options='-c standard_conforming_strings=off', host=_config.database_host, port=_config.database_port,
                        user=uname, password=_config.dsn_passwd, database=_config.database_name))
        ...
        def connect_unix(super_user=False):
            ...
            if not all_connections.has_key((database_name, uname)):
                all_connections[(database_name, uname)] = get_connection(_config, uname)
            ...
            try:
                ...
                close(rv)
                rv = all_connections[(database_name, uname)] = get_connection(_config, uname)
