...
def makeUpdateMNOpenAPIRequiest():
    api = openapi.OpenAPI()
    request = api.beginRequest()
...
def updateMNModules():
    ...
    openapi.waitRequestComplete(
        makeUpdateMNOpenAPIRequiest(),
        'Updating packages on MN',
        True)
    uAction.progress.done()
...
def updateBrands():
    openapi.OpenAPI().pem.refreshAllBrands()
