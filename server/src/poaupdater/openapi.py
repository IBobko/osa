...
class Transport(xmlrpc.SafeTransport):
    ...
    def __init__(self, config):
        can_init = '__init__' in dir(xmlrpc.SafeTransport)
        self.context_option_exist = can_init and 'context' in inspect.getargspec(xmlrpc.SafeTransport.__init__).args
        self.config = config
        # do not have any idea what does that mean:
        self._use_datetime = 0

        if self.context_option_exist and uLinux.SSL_VERIFICATION_ENFORCED:
            xmlrpc.SafeTransport.__init__(self, context=ssl._create_unverified_context())
        elif can_init:
            # python 2.7.5 in CentOS 7.4.1708 cannot obtain "context" option,
            # special method patch_ssl_context() will be used later
    ...
    def make_connection(self, host):
        ...
        self.set_connection_timeout(connection)
        need_patch_context = self.config.https and uLinux.SSL_VERIFICATION_ENFORCED and not self.context_option_exist

        if need_patch_context:
            self.patch_ssl_context(connection)

        return connection

    @staticmethod
    def patch_ssl_context(connection):
        # This monkey-patch need for python 2.7.5 in CentOS 7.4.1708
        # because SSL certificate verification is enabled by default (https://access.redhat.com/articles/2039753)
        # but bug #22960 is not ported from python 2.7.9 (https://bugs.python.org/issue22960)

        if "_context" not in dir(connection):
            uLogging.debug("Looks like patch for disabling strict SSL validation is needed, but it cannot be applied")
            return

        connection._check_hostname = connection._context.check_hostname = False
...
class OpenAPI:
    ...
    def __init__(self, settings=_settings, namespace=None):
        if settings==_settings and not _settings.initialized:
            # init from local config
            initFromEnv(uConfig.Config())
