...
class PBABaseConf(object):
    ...
    def configure(cls):
        ...
        request.command(cls.configureScript, stdout='stdout', stderr='stderr', retvar = 'retvar')
        ...
        if output['retvar'] != '0':
            uLogging.err("Command '%s' failed. Return code: %s, \n stdout: %s \n stderr: %s" % (cls.configureScript, output['retvar'], output['stdout'], output['stderr']))
            raise Exception("Command '%s' failed. Return code: %s, \n stdout: %s \n stderr: %s" % (cls.configureScript, output['retvar'], output['stdout'][-4048:], output['stderr'][-4048:]))
    ...
    def _start_stop(cls, command):
        if not command:
            ...
            return
        request = uHCL.Request(cls.get_host_id(), user='root', group='root')
        request.command(command, stdout='stdout', stderr='stderr', retvar = 'retvar')
        output = request.perform()
        if output['retvar'] != '0':
            if not (command == cls.stopCmd and output['retvar'] == '1' and 'STOPPED' in output['stdout']):
                raise Exception("Command '%s' failed. Return code: %s, \n stdout: %s \n stderr: %s" % (command, output['retvar'], output['stdout'], output['stderr']))
        uLogging.debug('done, output \n%s' % output['stdout'])
        return output
    ...
    def precheck(cls, lpath_to_precheck_tar = None):
        ...
        if lpath_to_precheck_tar: #else prechecks is assumed already at billing host
            ...
            request.mkdir(rpath_to_precheck)
            extract_cmd = ('tar -xf %s -C %s' % ( rpath_to_precheck_tar, rpath_to_precheck))
            request.command(extract_cmd, stdout='stdout', stderr='stderr', valid_exit_codes=[0])
...
class PBAConf(PBABaseConf):
    ...
    billingDBPropCmd = """grep -P '^ *%s *=' /usr/local/bm/etc/ssm.conf.d/.global.conf /usr/local/bm/etc/ssm.conf.d/global.conf | tail -1 | sed -e 's/.*= *//'"""
    billingAllDBPropCmd = """grep -P '^ *DB_' /usr/local/bm/etc/ssm.conf.d/%s | sed -e 's/ //g'"""
    ...
    @classmethod
    def getBillingDBProp(cls, propName):
        return uHCL.runHCLCmd(cls.get_host_id(), cls.billingDBPropCmd % propName).strip()

    @classmethod
    def getBillingDBPort(cls):
        return cls.getBillingDBProp('DB_PORT')

    @classmethod
    def getBillingDBHost(cls):
        return cls.getBillingDBProp('DB_HOST')

    @classmethod
    def getBillingDBUser(cls):
        return cls.getBillingDBProp('DB_USER')

    @classmethod
    def getBillingDBName(cls):
        return cls.getBillingDBProp('DB_NAME')

    @classmethod
    def getBillingDBPrams(cls):
        params = {}
        host_id = cls.get_host_id()
        if not host_id:
            return params
        for conf in ['.global.conf', 'global.conf']:
            config_props = uHCL.runHCLCmd(host_id, cls.billingAllDBPropCmd % conf)
            for p in map(lambda x: x.split('='), config_props.splitlines()):
                params[p[0]] = p[1]
        return params
...
def get_billing_platform_hosts():
    return _get_billing_hosts([PBAConf])

def get_billing_service_hosts():
    return _get_billing_hosts([PBAStoreConf])

def get_billing_hosts():
    res = get_billing_platform_hosts()
    res.update(get_billing_service_hosts())
    return res
...
UpgradeOrder = [PBAConf, PBAStoreConf]
...
def check_billing_node_free_disk_space(quota_in_gb):
    ...
    for node in billing_nodes:
        try:
            # checking space in /usr/local (/usr/local/bm & /usr/local/pem)
        ...
        try:
            # checking space in /var/log
            uPEM.check_free_disk_space(node.host_id, quota_in_gb, '/var/log')
        except uPEM.NotEnoughFreeDiskSpace, exception:
            reasons.append(exception.reason)
    if len(reasons) > 0:
