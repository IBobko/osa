...
def readCmdExt(command, input_data=None, env=None):
    ...
    use_shell = type(command) not in (tuple, list)
    if Const.isWindows():
        cmd = sp.Popen(command, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE, shell=use_shell, startupinfo=startup_info)
    else:
        cmd = sp.Popen(command, close_fds=True, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE, shell=use_shell, env=env)
...
def isLocalAddress(targetHostAddr):
    import uLinux
    targetHostIp = socket.gethostbyname(targetHostAddr)
    return targetHostIp in (x[1] for x in uLinux.listNetifaces())
...
def addToPropertiesFile(properties, filename):
    fl = open(filename)
    props_to_add = []
    props = readPropertiesFile(fl, to_array=True)
    for pname, pvalue in properties:
        propUpdated = False
        for i in range(len(props)):
            key, value = props[i]
            if key == pname:
                props[i] = (pname, pvalue)
                propUpdated = True
                break
        if not propUpdated:
            props_to_add.append((pname, pvalue))

    st = os.stat(filename)
    createPropertiesFile(props+props_to_add, filename, uid=st.st_uid, gid=st.st_gid)
...
class Tgz(object):
    ...
    @staticmethod
    def _unpack(src, dst):
        return readCmd(['tar', '--recursive-unlink', '-xzvf', src, '-C', dst])

    def getLocation(self):
        return self._location

    def unpackTo(self, outDirectory):
        return self._unpack(self._location, outDirectory)


class TgzArtifact(object):
    ...
    def unpackTo(self, outDirectory):
        return Tgz(self.getPath()).unpackTo(outDirectory)


class TgzArtifacts(object):

    def __init__(self, directory, filenames):
        self._directory = directory
        self._filenames = filenames
        self._tgzArtifacts = list(map(lambda name: TgzArtifact(self.getDirectory(), name), self.getFilenames()))
    ...
    def get(self):
        return self._tgzArtifacts

    def unpack(self):
        map(lambda tgz: tgz.unpackTo(self.getDirectory()), self.get())


class TimeZonesUpdater(object):

    def _exec(self, cmd):
        out = readCmd(cmd).strip()
        if out:
            uLogging.debug('Command output:\n%s' % out)
        return out

    def _create_workspace(self):
        return tempfile.mkdtemp(prefix='jdk_tzupdater_', suffix='_tzdata_rearguard')

    def _remove_workspace(self, path):
        if path:
            shutil.rmtree(path)

    def _get_distributive_path(self):
        return self._distributive_path

    def _get_updater_filename(self):
        return 'jdk-tzupdater.jar'

    def _get_updater_path(self):
        return os.path.join(self._get_distributive_path(), 'u', self._get_updater_filename())

    def _get_latest_tzdb_filename(self):
        return 'tzdb-latest.tar.lz'

    def _get_latest_tzdb_url(self):
        return 'http://www.iana.org/time-zones/repository/%s' % self._get_latest_tzdb_filename()

    def _get_latest_tzdb_path(self, workspace_path):
        return os.path.join(workspace_path, self._get_latest_tzdb_filename())

    def _get_tzdb_name_template(self):
        return 'tzdb-2*'

    def _get_tzdb_rearguard_template(self):
        return 'tzdata2*-rearguard.tar.gz'

    def _get_tzdb_rearguard_url(self, tzdb_rearguard_path):
        return 'file://%s' % tzdb_rearguard_path

    def _has_updater(self):
        return os.path.isfile(self._get_updater_path())

    def _download_latest_tzdb(self, workspace_path):
        self._exec('wget -P "%s" %s' % (workspace_path, self._get_latest_tzdb_url()))

    def _unpack_latest_tzdb(self, workspace_path, latest_tzdb_path):
        self._exec('bsdtar -C "%s" -xvf "%s"' % (workspace_path, latest_tzdb_path))

    def _find_tzdb(self, workspace_path):
        tzdb_name = self._exec('find "%s" -name "%s"' % (workspace_path, self._get_tzdb_name_template()))
        return os.path.join(workspace_path, tzdb_name)

    def _make_tzdb_rearguard(self, tzdb_path):
        self._exec('cd "%s" && make rearguard_tarballs' % tzdb_path)

    def _find_tzdb_rearguard(self, tzdb_path):
        return self._exec('find "%s" -name "%s"' % (tzdb_path, self._get_tzdb_rearguard_template()))

    def _update_timezone_rearguard(self, tzdb_rearguard_url):
        self._exec('java -jar "%s" -v -l "%s"' % (self._get_updater_path(), tzdb_rearguard_url))

    def _update_timezone(self, workspace_path):
        self._download_latest_tzdb(workspace_path)
        latest_tzdb_path = self._get_latest_tzdb_path(workspace_path)
        self._unpack_latest_tzdb(workspace_path, latest_tzdb_path)
        tzdb_path = self._find_tzdb(workspace_path)
        self._make_tzdb_rearguard(tzdb_path)
        tzdb_rearguard_path = self._find_tzdb_rearguard(tzdb_path)
        tzdb_rearguard_url = self._get_tzdb_rearguard_url(tzdb_rearguard_path)
        self._update_timezone_rearguard(tzdb_rearguard_url)

    def __init__(self, distributive_path):
        self._distributive_path = distributive_path
    ...
    def update_timezone(self):
        if self._has_updater():
            ...
            uLogging.debug('%s not found, skipping time zone update (%s)' %
...
__all__ = ["execCommand", "readCmd", "readCmdExt", "findHome", "PEMHost", "readPropertiesFile", "moveFile", "ln_cp",
           ...
           "replaceInFile", "nice_time_delta", "module_install_report", "stipPasswords", "is_secret_key", "capture_stderr",
           "TimeZonesUpdater"]
