...
TargetPostgreSQLVersion = "11" #default required PostgreSQL server version!
SupportedPostgreSQLVersions = ("9.0", "9.1", "9.2", "9.3", "9.4", "9.5", "9.6", "11")


PostgreSQLShowCmd = "su - postgres -c 'psql -t -P format=unaligned -c \"SHOW %s\" 2> /dev/null'"

def isPostgreSQLRunningLocal():
    return getPostgreSQLServerAddress()[1]


def getPostgreSQLServerAddress():
    con = uSysDB.connect()
    cur = con.cursor()
    cur.execute("SELECT inet_server_addr()")
    server_addr = cur.fetchone()[0]
    return server_addr, server_addr in (x[1] for x in uLinux.listNetifaces())
...
def getPostgreSQLVersionViaRPM(run):
    ver = None
    _vtrim = lambda vv: ".".join(vv.split(".")[:2])
    _vint = lambda vv: int("".join(vv.split(".")[:2]))
    _1stLine = lambda x: x.strip().splitlines()[0]

    postgresql_rpms = run("rpm -qa | grep -E \"postgresql(9[0-6]|11){0,1}-server\" || echo -n").strip().splitlines()
    for srv_rpm in postgresql_rpms:
        v = _vtrim(_1stLine(run("rpm -q --queryformat '%{version}' \""+srv_rpm+"\"")))
        if (ver is None) or (_vint(v) > _vint(ver)):
            ver = v
    return ver
...
class PostgreSQLConfig(object):
    ...
    def __init__(self, **kwargs):
        ...
        if "version" in kwargs:
            ...
            if ver and self.__ver == ".".join(ver.split(".")[:1]):
            ...
            if ver is None:
                ...
                show_cmd = PostgreSQLShowCmd
            if ver is None:
                ver = getPostgreSQLVersionViaRPM(self.__run)
                if ver is None:
                    raise Exception("Postgresql server is not installed on target node.")
            if ver:
                ver = ".".join(ver.split(".")[:-1])
            self.__ver = ver
        try:
            ver = _1stLine(self.__run("rpm -q --queryformat '%{version}' postgresql-server"))
            self.__verLess = self.__ver == ".".join(ver.split(".")[:-1])
        except:
    ...
    def get_version_as_int(self):
        return int("".join(self.__ver.split(".")[:1]))
    ...
    def set_autostart(self, enabled = True):
        if enabled:
            ...
            if self.__osVer[0] >= "7": #RHEL7 ?
                self.__run("systemctl disable " + self.get_service_name())
            else:
                self.__run("chkconfig " + self.get_service_name() + " off")
    ...
    def is_started(self):
        ...
        if ver is None:
            ver = getPostgreSQLRunningVersionViaRPM(self.__run)
        if ver:
            return self.__ver == ".".join(ver.split(".")[:-1])
        return False
