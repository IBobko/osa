...
def reinstallPackageToHost(host_id, name, ctype):
    ...
    api = openapi.OpenAPI()
    api.pem.packaging.reinstallPackageByNameSync(host_id=1, pname=name, ptype=ctype)
...
def installPackageToHost(host_id, name, ctype, properties=None):
    installPackageToHostAPI(host_id=host_id, name=name, ctype=ctype, properties=properties)
...
def _listImportedPackages():
    con = uSysDB.connect()
    cur = con.cursor()
    cur.execute("""
WITH
p AS (SELECT pkg_id, name, ctype, version, opsys, osrel, arch FROM packages JOIN platforms USING (platform_id)),
ps AS (SELECT pkg_id, value as is_single FROM packages p JOIN package_attributes pa USING (pkg_id) WHERE UPPER(pa.name) = 'COMPONENT.SINGLE'),
pc AS (SELECT pkg_id, value as is_custom FROM packages p JOIN package_attributes pa USING (pkg_id) WHERE UPPER(pa.name) = 'CUSTOMER')
SELECT pkg_id, name, ctype, version,
(CASE WHEN is_single IS NULL THEN 'no' WHEN is_single != 'yes' THEN 'no' ELSE 'yes' END)::BOOL as is_single,
(CASE WHEN is_custom IS NOT NULL THEN 'yes' ELSE 'no' END)::BOOL as is_custom,
opsys, osrel, arch
FROM p LEFT JOIN ps USING (pkg_id) LEFT JOIN pc USING (pkg_id)
    """)
    return cur.fetchall()
...
def _getNewestPackages(rv, build, packages, filt=None):
    for pkg in packages:
        ...
        if not (rv[platform][ctype].has_key(pkg.package.name)) or (uBuild.compare_versions(rv[platform][ctype][name].version, pkg.version) <= 0):
            pkg.tarball_location = None if not build else build.find_valid_tarball(pkg)
...
def getNewestPackages(builds, filt=None):
    ...
    rv = {}
    for build in builds:
        _getNewestPackages(rv, build, build.contents.values(), filt)
    return rv


def getImportedPackages():
    uLogging.debug("searching for imported packages to install")
    rv = {}
    _getNewestPackages(rv, None, listImportedPackages())
    return rv
...
def deployBaseExesFiles(builds):
    for build in list(reversed(builds)):
        async_exec_path = build.windows_update.async_exec_path
        sn_installer_path = build.windows_update.sn_installer_path
        if async_exec_path and sn_installer_path:
            _deployBaseExesFiles([async_exec_path, sn_installer_path])
            return
...
def clean_interfaces():
    ...
    con.commit()
    cur.execute(
        "SELECT p.pkg_id, p.name, p.ctype, i.service_type FROM interfaces i JOIN dep_interfaces di ON (i.interface_id = di.interface_id) JOIN package_dependencies dp ON (di.dep_id = dp.dep_id) JOIN packages p ON (dp.pkg_id = p.pkg_id) WHERE i.interface_id NOT IN (SELECT interface_id FROM package_interfaces)")
    pkgs = {}
    for row in cur.fetchall():
...
class RepoTarballs(object):
    ...
    def __init__(self):
        self.__con = RepoTarballs.__getDbConnection()

    def getInfo(self, fileName):
        cur = self.__con.cursor()
        cur.execute('SELECT name, ctype, version, pkg_id, platform_id FROM packages WHERE filename = %s', fileName)
        return cur.fetchall()

    def getVersions(self, name, ctype, platform_id):
        cur = self.__con.cursor()
        cur.execute('SELECT version FROM packages WHERE name = %s and ctype=%s AND platform_id = %s',
                    name, ctype, platform_id)
        return cur.fetchall()

    def isLegacy(self, fileName):
        rows = self.getInfo(fileName)
        if not rows:
            uLogging.debug('removing %s: unused', fileName)
            return True
        row = rows[0]
        name, ctype, version, pkg_id, platform_id = row[0], row[1], row[2], row[3], row[4]
        otherVersions = self.getVersions(name, ctype, platform_id)
        higherVersion = uGenerics.first(otherVersions, lambda row: RepoTarballs.compareVersions(row[0], version) > 0)
        if higherVersion:
            uLogging.debug('removing %s: higher version %s exists', fileName, higherVersion[0])
            return True
        return False

    def listMirror(self, localpath):
        files = []
    ...
    def cleanMirror(self, localpath):
        for fileName, filePath in self.listMirror(localpath):
            if self.isLegacy(fileName):
                os.unlink(filePath)

class ArchChangedPackages(object):
    ...
    def _getAnyAnyAnyPlatformId(self):
        return getPlatformId(self._con, ('any', 'any', 'any'))

    def _find(self, anyAnyAnyPlatformId):
        cur = self._con.cursor()
        cur.execute("""WITH detector AS (
                           SELECT p.name, p.ctype, COUNT(DISTINCT p.platform_id), BOOL_OR(p.platform_id = %s)
                           FROM packages p
                           GROUP BY p.name, p.ctype
                           HAVING COUNT(DISTINCT p.platform_id) > 1 AND BOOL_OR(p.platform_id = %s)
                       ), selector AS (
                           SELECT p.name, p.ctype, p.platform_id, p.pkg_id, p.version
                           FROM packages p
                       )
                       SELECT s.name, s.ctype, s.platform_id, s.pkg_id, s.version,
                       CASE WHEN EXISTS (SELECT 1 FROM components c WHERE c.pkg_id = s.pkg_id) THEN 1 ELSE 0 END
                       FROM detector d, selector s
                       WHERE s.name = d.name AND s.ctype = d.ctype""", anyAnyAnyPlatformId, anyAnyAnyPlatformId)
        return cur.fetchall()

    def _createIndex(self, archChangedPackages):
        packages = {}
        for record in archChangedPackages:
            name, ctype, platformId, pkgId, version, hasComponent = record
            package = name, ctype
            if package not in packages:
                packages[package] = {}
            if platformId not in packages[package]:
                packages[package][platformId] = []
            packages[package][platformId].append((pkgId, version, hasComponent))
        return packages

    def _getHighestVersion(self, index, package, platformId):
        vHighest = None
        for version in index[package][platformId]:
            if vHighest is None:
                vHighest = version
            else:
                if 0 > ArchChangedPackages.compareVersions(vHighest[1], version[1]):
                    vHighest = version
        return vHighest

    def _getBoundaryVersions(self, index, package):
        pHighest = None
        pLowest = None
        pHighestId = None
        pLowestId = None
        for platformId in index[package]:
            vHighest = self._getHighestVersion(index, package, platformId)
            if pHighest is None:
                pHighest, pHighestId = vHighest, platformId
            else:
                if 0 > ArchChangedPackages.compareVersions(pHighest[1], vHighest[1]):
                    pHighest, pHighestId = vHighest, platformId
            if pLowest is None:
                pLowest, pLowestId = vHighest, platformId
            else:
                if 0 < ArchChangedPackages.compareVersions(pLowest[1], vHighest[1]):
                    pLowest, pLowestId = vHighest, platformId
        return pHighest, pLowest, pHighestId, pLowestId

    def _getIds(self, index, package, platformId):
        legacy = []
        for version in index[package][platformId]:
            if version[2]:
                continue
            legacy.append(version[0])
            uLogging.info('Legacy arch-changed package: %s %s %s' % (package, platformId, version))
        return legacy

    def _getLegacyIds(self, index, package, anyAnyAnyPlatformId, pHighest, pLowest, pHighestId, pLowestId):
        legacy = []
        if pHighestId != pLowestId:
            if pLowestId == anyAnyAnyPlatformId:
                legacy.extend(self._getIds(index, package, anyAnyAnyPlatformId))
            if pHighestId == anyAnyAnyPlatformId:
                for platformId in index[package]:
                    if platformId != anyAnyAnyPlatformId:
                        legacy.extend(self._getIds(index, package, platformId))
        return legacy

    def __init__(self):
        self._con = ArchChangedPackages._getDbConnection()

    def getLegacyIds(self):
        legacy = []
        anyAnyAnyPlatformId = self._getAnyAnyAnyPlatformId()
        archChangedPackages = self._find(anyAnyAnyPlatformId)
        index = self._createIndex(archChangedPackages)
        for package in index:
            pHighest, pLowest, pHighestId, pLowestId = self._getBoundaryVersions(index, package)
            legacy.extend(self._getLegacyIds(index, package, anyAnyAnyPlatformId, pHighest, pLowest, pHighestId, pLowestId))
        return legacy
