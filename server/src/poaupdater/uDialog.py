...
def input(query_str, timeout=None):
    while True:
        try:
            print query_str,
            sys.stdout.flush()
            if timeout:
               i, o, e = sele.select([sys.stdin], [], [], timeout)
               if not i:
                   return ''
            return raw_input()
        except EOFError:
...
def askYesNo(question=None, default=None, timeout=None):

    query_string = '\n((Y)es/(N)o)'

    if question:
        query_string = "\n{question}{query_string}".format(question=question, query_string=query_string)

    if default is not None:

        if default:
            query_string += " [default: yes]"
        else:
            query_string += " [default: no]"
    ...
    while True:
        answer = input(query_string, timeout)
