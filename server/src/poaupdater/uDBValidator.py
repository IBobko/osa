...
def validateDatabaseExt(manifest_tables, tables, skip_constraint_names=False, check_triggers=False):
...
def _recreate_verify_db(config_v):
    # Drop and create verify DB
    assert config_v.database_name.endswith('_verify'), "Can't re-create database. DB name is %s, which is not verify DB!" % config_v.database_name
    if config_v.admin_db_password:
       from poaupdater import uOSCommon
       uOS = uOSCommon.getUOSModule()
       uOS.db_admin_query(config_v, uLinux.getDropDbSql(config_v.database_name), config_v.admin_db)
       uOS.db_admin_query(config_v, uLinux.getCreateDbSql(config_v.database_name), config_v.admin_db)
    else:
       sqlRunner = uDBAccessAPI.getSqlCommandRunner(config_v.database_host)
       sqlRunner(uLinux.getDropDbSql(config_v.database_name))
       sqlRunner(uLinux.getCreateDbSql(config_v.database_name))

def _cleanup_verify_db(config_v):
    uLogging.info('Clear reference DB')
    uSysDB.init(config_v)
    assert config_v.database_name.endswith('_verify'), "Can't clear database. DB name is %s, which is not verify DB!" % config_v.database_name
...
def initialize_verify_db(binfo, config=None):
    ...
    import imp
    migrateDB = imp.load_source('migrateDB', os.path.join(j.pau_db_migration, 'migrateDB.py'))
...
def validateDatabase(binfo, config=None):
    ...
    manifest_tables.update(readTablesFromDB(con_v))
    uSysDB.close(con_v)
...
class DBValidator:
    ...
    def __init__(self, skip_constraint_names = False, check_triggers = False):
        self.skip_constraint_names = skip_constraint_names
        self.check_triggers = check_triggers
        ...
        for schema_name in schemas:
            ...
            if schema_name in tables:
                ...
                if self.check_triggers:
                    errors += self._validateTriggers(schemas[schema_name], tables[schema_name])
...
#						errors.append(DBErrorRedundantForeignKey(table, constraint))
    ...
    def _validateTriggers(self, schema, table):
        expectedTriggers = Set(schema.triggers)
        actualTriggers = Set(table.triggers)

        errors = []
        if expectedTriggers != actualTriggers:
            errors.append(DBErrorMismatchTriggers(table, schema.triggers))
