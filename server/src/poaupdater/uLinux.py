...
if '_create_unverified_context' in dir(ssl):
    SSL_VERIFICATION_ENFORCED = True
else:
    SSL_VERIFICATION_ENFORCED = False
...
def _getDeterminePlatformCommand():
    return "python -c 'import platform; print %s.join(platform.linux_distribution() + (platform.machine(),))'" % '";"'


def determineNodePlatform(node_id):
    result = uHCL.runHCLCmd(node_id, _getDeterminePlatformCommand())
    distname, version, _, arch = result.strip().split(';')
    return uBuild._create_platform(distname, version, arch)
...
def check_hostname_resolution():
    # Check that 'localhost' can be resolved
    try:
        socket.gethostbyname('localhost')
    except Exception, e:
        raise Exception("Hostname 'localhost' could not be resolved. Please fix /etc/hosts.")

    # check forward backward name resolution, pau will not be able to properly init corba servants otherwise
    start = time.time()
    try:
        socket.gethostbyname(socket.getfqdn())
    except Exception, e:
        raise Exception("Error in hostname resolution (hostname -i): %s" % e)
    finish = time.time()
    if (finish - start) > 1.0:
        raise Exception("Hostname resolution (hostname -i) work with delay > 1 second. It can cause instabilities during startup. Please fix hostname resolution confguration: /etc/hosts, /etc/resolv.conf")

    uLogging.debug("hostname resolution: OK")
...
def initDefaultProps(config):
    ...
    config.admin_db_user = "postgres"
    config.admin_db_password = None
    config.admin_db = "template1"
...
def is_yum_package_provided(package_name):
    import yum
    try:
        yb = yum.YumBase()
        yb.preconf.init_plugins = False
        ...
        matches = yb.searchGenerator(fields, criteria)
        for (package, _) in matches:
...
def tunePostgresLogs(run):
    ...
    uLogging.info("Alter logging configuration")
    ...
    run("""su - postgres -c "psql -t -P format=unaligned -c $'alter system set %s = %s '" """ % ('log_autovacuum_min_duration', r"\'1s\'"))
...
def tunePostgresParams(pgsql, config):
    ...
    tunePostgresLogs(pgsql.get_commander())
...
def tunePostgresParamsImpl(pgsql):
    run = pgsql.get_commander()

    uLogging.info("Running odin-pg-tune to generate target postgresql configuration file with respect to available memory and required number of connections...")
    pg_conf = pgsql.get_postgresql_conf()
    pg_conf_new  = pg_conf + ".new"

    run("echo 'yes' | cp " + pg_conf + " " + pg_conf_new)
    run("odin-pg-tune -f --input-config=" + pg_conf + " --min-connections=256 --output-config=" + pg_conf_new)
    ...
    run("chown postgres: " + pg_conf_new)
...
def getCreateDbSql(database_name, clean=False):
    template = 'template1'
    if clean:
        template = 'template0'
    return "CREATE DATABASE %s ENCODING 'UTF8' TEMPLATE %s" % (database_name, template)


def getDropDbSql(database_name):
    return 'DROP DATABASE IF EXISTS %s' % database_name
...
def __db_connect(user, database_name, database_host=None, password=None, database_port=None):
    if database_host is None:
        ...
        uLogging.debug('connecting to database, user %s, database_name %s, host %s' % (user, database_name, database_host))
    import psycopg2
    max_tries = 60
    i = 0
    while True:
        try:
            admin_con = psycopg2.connect(user=user, database=database_name, host=database_host, port=database_port, password=password)
            break
...
def db_admin_query(config, query, database_name=None):
    if not database_name:
        database_name = config.database_name

    if config.admin_db_password is None:
        # connect locally
        admin_con = __db_connect(config.admin_db_user, database_name)
    else:
        admin_con = __db_connect(config.admin_db_user, database_name, config.database_host, config.admin_db_password, config.database_port)
    try:
...
def create_db(config, askYesNo, clean=False):
    ...
    if config.admin_db_password is None:
        # connect locally
        admin_con = __db_connect(config.admin_db_user, config.admin_db)
    else:
        admin_con = __db_connect(config.admin_db_user, config.admin_db, config.database_host, config.admin_db_password, config.database_port)
    try:
        cur = admin_con.cursor()
        cur.execute("SELECT datname FROM pg_database WHERE datname='%s'" % config.database_name)
        if cur.fetchone():
            uLogging.debug("Database '%s' already exist" % config.database_name)
            ...
            uLogging.debug("Database '%s' does not exist" % config.database_name)

        cur.execute("SELECT 1 FROM pg_proc WHERE proname = 'plpgsql_call_handler' AND pronargs=0")
        if not cur.fetchone():
            cur.execute(
        ...
        cur = admin_con.cursor()
        cur.execute(getCreateDbSql(config.database_name, clean))
        cur.execute("SELECT usesysid FROM pg_shadow WHERE usename = '%s'" % dsn_login_short)
        row = cur.fetchone()
        if not row:
            cur.execute("CREATE USER %s WITH PASSWORD '%s' NOCREATEDB; ALTER ROLE %s SET transform_null_equals TO on;" % (dsn_login_short, config.dsn_passwd, dsn_login_short))
            cur.execute("SELECT usesysid FROM pg_shadow WHERE usename = '%s'" % dsn_login_short)
            row = cur.fetchone()
        dbuser_oid = row[0]
        # For cloud: admin user needs to be granted access to dns_login role
        cur.execute("GRANT %s TO %s" % (dsn_login_short, admin_db_user_short))
        cur.execute("ALTER DATABASE %s OWNER TO %s" % (config.database_name, dsn_login_short))
        if config.dsn_passwd:
            cur.execute("ALTER USER %s WITH ENCRYPTED PASSWORD '%s';" % (dsn_login_short, config.dsn_passwd))
    finally:
        __set_uid_by_user('root')
        admin_con.close()
...
def dropDB(config):
    ...
    cur = admin_con.cursor()
    cur.execute(getDropDbSql(config.database_name))
    __set_uid_by_user('root')
