...
class DBErrorMismatchTriggers(DBError):

    def __init__(self, _table, _expected_triggers):
        self.table = _table
        self.expected_triggers = _expected_triggers
        self.name = "triggers mismatch"

    def __str__(self):
        return "%s on table '%s': \"%s\", must be \"%s\"" % (self.name, self.table.name, self.table.triggers, self.expected_triggers)
