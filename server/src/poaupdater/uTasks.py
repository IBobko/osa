...
class DBTaskManagement:
    ...
    def schedulePeriodic(self, task):
        ...
        task_id = self.__scheduleCommon(task)
        cur = self.con.cursor()
        cur.execute("UPDATE tm_tasks SET non_legacy = 1 WHERE task_id = %s", task_id)
        cur.execute("INSERT INTO tm_periodic(task_id, period_type, start_at_hour, start_at_min, start_at_day, period) VALUES(%s, %s, %s, %s, %s, %s)", (task_id,) + task.period)
        cur.execute("INSERT INTO tm_params(task_id, p_name, p_value) VALUES (%s, %s, %s)", task_id, 'JNDI_NAME', 'java:global/core-ear/job-ejb/PeriodicCorbaTaskProcessorBean!com.parallels.pa.service.periodic.api.PeriodicCorbaTaskProcessor')
        return task_id
...
upgraderModuleGroupPattern = 'Install_Module%'

def unfinishedTaskGroup(gclassPattern):
    con = uSysDB.connect()
    cur = con.cursor()
    cur.execute("SELECT t.gclass FROM tm_groups t WHERE t.gclass LIKE %s ", gclassPattern)
    return [row[0] for row in cur.fetchall()]

def isModuleUpdateFinished():
    return not unfinishedTaskGroup(upgraderModuleGroupPattern)
...
def waitTaskGroupComplete(gclassPattern):
    ...
    while True:
        groups = unfinishedTaskGroup(gclassPattern)
        if not groups:
            break
